...
public plugin_init() {
    ...
    register_plugin("Admin Connection Check", "1.0.1", "Daniel");
	...
	register_clcmd("say /checkme", "CMD_CheckMe");
    register_clcmd("say_team /checkme", "CMD_CheckMe");
	register_clcmd("say checkme", "CMD_CheckMe");
    register_clcmd("say_team checkme", "CMD_CheckMe");
	
	set_task(REMINDER_INTERVAL, "SENT_Reminder", 0, "", 0, "b"); 
...
public CMD_CheckMe(id) {
    if (is_user_admin(id)) {
        client_print(id, print_chat, "Admin is ON");
    } else {
        client_print(id, print_chat, "Admin is OFF");
    }
}

stock calculate_inactive_days(const last_seen[]) {
    new date[32];
    get_time("%d %B %y", date, 31);

    new last_day, last_month, last_year;
    parse_date(last_seen, last_day, last_month, last_year);

    new current_day, current_month, current_year;
    parse_date(date, current_day, current_month, current_year);

    new last_total_days = date_to_days(last_day, last_month, last_year);
    new current_total_days = date_to_days(current_day, current_month, current_year);

    return current_total_days - last_total_days;
}


stock parse_date(const date[], &day, &month, &year) {
    new month_str[16];
    parse(date, "%d %s %d", day, 31, month_str, 15, year, 31);
    month = month_to_number(month_str);
}


stock month_to_number(const month_str[]) {
    if (equal(month_str, "January")) return 1;
    if (equal(month_str, "February")) return 2;
    if (equal(month_str, "March")) return 3;
    if (equal(month_str, "April")) return 4;
    if (equal(month_str, "May")) return 5;
    if (equal(month_str, "June")) return 6;
    if (equal(month_str, "July")) return 7;
    if (equal(month_str, "August")) return 8;
    if (equal(month_str, "September")) return 9;
    if (equal(month_str, "October")) return 10;
    if (equal(month_str, "November")) return 11;
    if (equal(month_str, "December")) return 12;
    return 0;
}


stock date_to_days(day, month, year) {
    new days_in_months[12] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
    new total_days = (year - 2000) * 365 + (year - 2000) / 4; // Including leap years
...
public CMD_AdminConnection(id) {
    ...
    switch (SELECT_DESIGN) {
		...
		while (fgets(FileOpen, Line, 127)) {
			...
			new inactive_days = calculate_inactive_days(Arg2);
